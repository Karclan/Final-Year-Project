...
void SceneTest::init()
{
	...
	m_Renderer.init();
	m_InputHandler.init();
	m_ControllerInput.init(0);

	SPC_Transform  bT(m_TransformManager.createTransform(glm::vec3(0.f, 0.f, -70.f), glm::vec3(0.f, 0.f, 0.f), glm::vec3(200.f, 200.f, 0.1f)));
	SPC_Collidable bC(m_CollisionManager.createCollidable(bT,1.0, 1.0, GameObjectType::BLOCK));
	SPC_Renderable bR(m_Renderer.createRenderable(bT,"cube2.obj","space.png"));
	...
	bC = (m_CollisionManager.createCollidable(bT, 1.0, 1.0, GameObjectType::BLOCK));
	...
	SPC_Collidable fc(m_CollisionManager.createCollidable	(ft, 1.0, 2.0, GameObjectType::BLOCK));
	...
	//Platform 1
	SPC_Transform  pft0(m_TransformManager.createTransform(glm::vec3(8.f, 4.f, 0.f), glm::vec3(90.f, 0.f, 0.f), glm::vec3(4.f,1.f,1.f)));
	SPC_Collidable pfc0(m_CollisionManager.createCollidable(pft0, 4.0,1.0, GameObjectType::BLOCK));
	...
	//Platform 2
	SPC_Transform  pft1(m_TransformManager.createTransform(glm::vec3(0.f, 8.f, 0.f), glm::vec3(90.f,0.f,0.f), glm::vec3(4.f,1.f,1.f)));
	SPC_Collidable pfc1(m_CollisionManager.createCollidable(pft1, 4.0,1.0, GameObjectType::BLOCK));
	...
	//Platform 3
	SPC_Transform  pft2(m_TransformManager.createTransform(glm::vec3(-8.f, 4.f, 0.f), glm::vec3(90.f, 0.f, 0.f), glm::vec3(4.f, 1.f, 1.f)));
	SPC_Collidable pfc2(m_CollisionManager.createCollidable(pft2, 4.0, 1.0, GameObjectType::BLOCK));
	...
	SPC_Collidable f1c(m_CollisionManager.createCollidable(f1t, 1.25,1.25, GameObjectType::FIGHTER));
	...
	f1r->setSpecEx(64.f);
	SPC_PhysicsBody f1pb(m_PhysicsManager.createPhysicsBody(f1t, f1c));
	m_player1 = new Fighter(5.f, f1t, f1r, f1c,f1pb);
	m_GameObjectManager.addGO(m_player1);
	//CAMERA
	SPC_Transform  st(m_TransformManager.createTransform(glm::vec3(0.f,5.f, 20.f), glm::vec3(0.f), glm::vec3(1.f)));
	...
	*/
	...
	SPC_Transform fireworkT(m_TransformManager.createTransform(glm::vec3(0.f, 2.f, -5.f), glm::vec3(0.f), glm::vec3(1.f)));
...
void SceneTest::handleInput()
{
	...
	{
		SPC_PhysicsBody t = std::dynamic_pointer_cast<PhysicsBody>(m_player1->findComponent(ComponentType::PHYSICSBODY));
		t->addImpulse(glm::vec3(0.f, 35.0f*Timer::getTime().asSeconds(), 0.f));
	}
...
void SceneTest::handleController()
{
	...
	{
		SPC_PhysicsBody t = std::dynamic_pointer_cast<PhysicsBody>(m_player1->findComponent(ComponentType::PHYSICSBODY));
		t->addImpulse(glm::vec3(0.f, 25.0f*Timer::getTime().asSeconds(), 0.f));
	}
	if (m_ControllerInput._leftStick.x < -10.f)
	{
		SPC_PhysicsBody t = std::dynamic_pointer_cast<PhysicsBody>(m_player1->findComponent(ComponentType::PHYSICSBODY));
		t->addImpulse(glm::vec3(-5.0f*Timer::getTime().asSeconds(), 0.f, 0.f));
	}
	...
	if (m_ControllerInput._leftStick.y < -10.f)
	{
		SPC_PhysicsBody t = std::dynamic_pointer_cast<PhysicsBody>(m_player1->findComponent(ComponentType::PHYSICSBODY));
		t->addImpulse(glm::vec3(0.f, -5.0f*Timer::getTime().asSeconds(), 0.f));
	}
